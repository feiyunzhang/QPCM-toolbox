...
class resnet_v1_101_cascade_rcnn(Symbol):
    def __init__(self):
        """
    ...
    def get_symbol(self, cfg, is_train=True):
        ...
        if is_train:
            ...
            rois, label, bbox_target, bbox_weight = mx.sym.Custom(rois=rois, gt_boxes=gt_boxes_reshape,
                                                                  ...
                                                                  fg_fraction=cfg.TRAIN.FG_FRACTION, stage=1)
        else:
            # ROI Proposal
            rpn_cls_score_reshape = mx.sym.Reshape(
                data=rpn_cls_score, shape=(0, 2, -1, 0), name="rpn_cls_score_reshape")
        ...
        # loss
        if is_train:
            cls_prob1 = mx.sym.SoftmaxOutput(name='cls_prob1', data=cls_score, label=label, normalization='valid')
            bbox_loss_1 = bbox_weight * mx.sym.smooth_l1(name='bbox_loss_1', scalar=1.0,
                                                        data=(bbox_pred - bbox_target))
            bbox_loss1 = mx.sym.MakeLoss(name='bbox_loss1', data=bbox_loss_1, grad_scale=1.0 / cfg.TRAIN.BATCH_ROIS)
            rcnn_label1 = label
            # reshape output
            rcnn_label1 = mx.sym.Reshape(data=rcnn_label1, shape=(cfg.TRAIN.BATCH_IMAGES, -1), name='label_reshape1')
            cls_prob1 = mx.sym.Reshape(data=cls_prob1, shape=(cfg.TRAIN.BATCH_IMAGES, -1, num_classes),
                                      name='cls_prob_reshape1')
            bbox_loss1 = mx.sym.Reshape(data=bbox_loss1, shape=(cfg.TRAIN.BATCH_IMAGES, -1, 4 * num_reg_classes),
                                       name='bbox_loss_reshape1')
        else:
            pass
        ...
        '''cascade 2'''
        if is_train:
            proposals2 = mx.sym.Custom(
                rois=rois, bbox_deltas=bbox_pred, im_info=im_info,
                op_type='decode_bbox') # 128*5, absolute coordinate
            rois2, label2, bbox_target2, bbox_weight2 = mx.sym.Custom(rois=proposals2, gt_boxes=gt_boxes_reshape,
                                                                  op_type='proposal_target',
                                                                  num_classes=num_reg_classes,
                                                                  batch_images=cfg.TRAIN.BATCH_IMAGES,
                                                                  batch_rois=cfg.TRAIN.BATCH_ROIS,
                                                                  cfg=cPickle.dumps(cfg),
                                                                  fg_fraction=cfg.TRAIN.FG_FRACTION, stage=2)
            # rois2: 128*5, absolute coordinate
        else:
            rois2 = mx.sym.Custom(
                rois=rois, bbox_deltas=bbox_pred, im_info=im_info, name='rois2',
                op_type='decode_bbox')  # 128*5, absolute coordinate
        roi_pool2 = mx.symbol.ROIPooling(
            name='roi_pool2', data=conv_new_1_relu, rois=rois2, pooled_size=(7, 7), spatial_scale=0.0625)
        # 2 fc
        fc_new_12 = mx.symbol.FullyConnected(name='fc_new_12', data=roi_pool2, num_hidden=1024)
        fc_new_1_relu2 = mx.sym.Activation(data=fc_new_12, act_type='relu', name='fc_new_1_relu2')

        fc_new_22 = mx.symbol.FullyConnected(name='fc_new_22', data=fc_new_1_relu2, num_hidden=1024)
        fc_new_2_relu2 = mx.sym.Activation(data=fc_new_22, act_type='relu', name='fc_new_2_relu2')
        # cls_score/bbox_pred
        cls_score2 = mx.symbol.FullyConnected(name='cls_score2', data=fc_new_2_relu2, num_hidden=num_classes)
        bbox_pred2 = mx.symbol.FullyConnected(name='bbox_pred2', data=fc_new_2_relu2, num_hidden=num_reg_classes * 4)

        # loss
        if is_train:
            cls_prob2 = mx.sym.SoftmaxOutput(name='cls_prob2', data=cls_score2, label=label2, normalization='valid')
            bbox_loss_2 = bbox_weight2 * mx.sym.smooth_l1(name='bbox_loss_2', scalar=1.0,
                                                          data=(bbox_pred2 - bbox_target2))
            bbox_loss2 = mx.sym.MakeLoss(name='bbox_loss2', data=bbox_loss_2, grad_scale=1.0 / cfg.TRAIN.BATCH_ROIS)
            rcnn_label2 = label2
            # reshape output
            rcnn_label2 = mx.sym.Reshape(data=rcnn_label2, shape=(cfg.TRAIN.BATCH_IMAGES, -1), name='label_reshape2')
            cls_prob2 = mx.sym.Reshape(data=cls_prob2, shape=(cfg.TRAIN.BATCH_IMAGES, -1, num_classes),
                                       name='cls_prob_reshape2') * 0.5
            bbox_loss2 = mx.sym.Reshape(data=bbox_loss2, shape=(cfg.TRAIN.BATCH_IMAGES, -1, 4 * num_reg_classes),
                                        name='bbox_loss_reshape2') * 0.5
        else:
            pass
            # cls_prob2 = mx.sym.SoftmaxActivation(name='cls_prob2', data=cls_score2)
            # cls_prob2 = mx.sym.Reshape(data=cls_prob2, shape=(cfg.TEST.BATCH_IMAGES, -1, num_classes),
            #                           name='cls_prob_reshape2')
            # bbox_pred2 = mx.sym.Reshape(data=bbox_pred2, shape=(cfg.TEST.BATCH_IMAGES, -1, 4 * num_reg_classes),
            #                            name='bbox_pred_reshape2')
            # group = mx.sym.Group([rois2, cls_prob2, bbox_pred2])

        '''cascade 3'''
        if is_train:
            proposals3 = mx.sym.Custom(
                rois=rois2, bbox_deltas=bbox_pred2, im_info=im_info,
                op_type='decode_bbox')
            rois3, label3, bbox_target3, bbox_weight3 = mx.sym.Custom(rois=proposals3, gt_boxes=gt_boxes_reshape,
                                                                      op_type='proposal_target',
                                                                      num_classes=num_reg_classes,
                                                                      batch_images=cfg.TRAIN.BATCH_IMAGES,
                                                                      batch_rois=cfg.TRAIN.BATCH_ROIS,
                                                                      cfg=cPickle.dumps(cfg),
                                                                      fg_fraction=cfg.TRAIN.FG_FRACTION, stage=3)
        else:
            rois3 = mx.sym.Custom(
                rois=rois2, bbox_deltas=bbox_pred2, im_info=im_info, name = 'rois3',
                op_type='decode_bbox')
        roi_pool3 = mx.symbol.ROIPooling(
            name='roi_pool3', data=conv_new_1_relu, rois=rois3, pooled_size=(7, 7), spatial_scale=0.0625)
        # 2 fc
        fc_new_13 = mx.symbol.FullyConnected(name='fc_new_13', data=roi_pool3, num_hidden=1024)
        fc_new_1_relu3 = mx.sym.Activation(data=fc_new_13, act_type='relu', name='fc_new_1_relu3')
        fc_new_23 = mx.symbol.FullyConnected(name='fc_new_23', data=fc_new_1_relu3, num_hidden=1024)
        fc_new_2_relu3 = mx.sym.Activation(data=fc_new_23, act_type='relu', name='fc_new_2_relu3')
        # cls_score/bbox_pred
        cls_score3 = mx.symbol.FullyConnected(name='cls_score3', data=fc_new_2_relu3, num_hidden=num_classes)
        bbox_pred3 = mx.symbol.FullyConnected(name='bbox_pred3', data=fc_new_2_relu3, num_hidden=num_reg_classes * 4)

        # loss
        if is_train:
            cls_prob3 = mx.sym.SoftmaxOutput(name='cls_prob3', data=cls_score3, label=label3, normalization='valid')
            bbox_loss_3 = bbox_weight3 * mx.sym.smooth_l1(name='bbox_loss_3', scalar=1.0,
                                                          data=(bbox_pred3 - bbox_target3))
            bbox_loss3 = mx.sym.MakeLoss(name='bbox_loss3', data=bbox_loss_3, grad_scale=1.0 / cfg.TRAIN.BATCH_ROIS)
            rcnn_label3 = label3
            # reshape output
            rcnn_label3 = mx.sym.Reshape(data=rcnn_label3, shape=(cfg.TRAIN.BATCH_IMAGES, -1), name='label_reshape3')
            cls_prob3 = mx.sym.Reshape(data=cls_prob3, shape=(cfg.TRAIN.BATCH_IMAGES, -1, num_classes),
                                       name='cls_prob_reshape3') * 0.25
            bbox_loss3 = mx.sym.Reshape(data=bbox_loss3, shape=(cfg.TRAIN.BATCH_IMAGES, -1, 4 * num_reg_classes),
                                        name='bbox_loss_reshape3') * 0.25

        if is_train:
            group = mx.sym.Group([rpn_cls_prob, rpn_bbox_loss,
                                  cls_prob1, bbox_loss1, mx.sym.BlockGrad(rcnn_label1),
                                  cls_prob2, bbox_loss2, mx.sym.BlockGrad(rcnn_label2),
                                  cls_prob3, bbox_loss3, mx.sym.BlockGrad(rcnn_label3)])
        else:
            # pass
            cls_prob3 = mx.sym.SoftmaxActivation(name='cls_prob3', data=cls_score3)
            cls_prob3 = mx.sym.Reshape(data=cls_prob3, shape=(cfg.TEST.BATCH_IMAGES, -1, num_classes),
                                      name='cls_prob_reshape3')
            bbox_pred3 = mx.sym.Reshape(data=bbox_pred3, shape=(cfg.TEST.BATCH_IMAGES, -1, 4 * num_reg_classes),
                                       name='bbox_pred_reshape3')
            group = mx.sym.Group([rois3, cls_prob3, bbox_pred3])
    ...
    def init_weight_rcnn(self, cfg, arg_params, aux_params):
        ...
        arg_params['fc_new_12_weight'] = mx.random.normal(0, 0.01, shape=self.arg_shape_dict['fc_new_1_weight'])
        arg_params['fc_new_12_bias'] = mx.nd.zeros(shape=self.arg_shape_dict['fc_new_1_bias'])
        arg_params['fc_new_22_weight'] = mx.random.normal(0, 0.01, shape=self.arg_shape_dict['fc_new_2_weight'])
        arg_params['fc_new_22_bias'] = mx.nd.zeros(shape=self.arg_shape_dict['fc_new_2_bias'])
        arg_params['cls_score2_weight'] = mx.random.normal(0, 0.01, shape=self.arg_shape_dict['cls_score_weight'])
        arg_params['cls_score2_bias'] = mx.nd.zeros(shape=self.arg_shape_dict['cls_score_bias'])
        arg_params['bbox_pred2_weight'] = mx.random.normal(0, 0.01, shape=self.arg_shape_dict['bbox_pred_weight'])
        arg_params['bbox_pred2_bias'] = mx.nd.zeros(shape=self.arg_shape_dict['bbox_pred_bias'])

        arg_params['fc_new_13_weight'] = mx.random.normal(0, 0.01, shape=self.arg_shape_dict['fc_new_1_weight'])
        arg_params['fc_new_13_bias'] = mx.nd.zeros(shape=self.arg_shape_dict['fc_new_1_bias'])
        arg_params['fc_new_23_weight'] = mx.random.normal(0, 0.01, shape=self.arg_shape_dict['fc_new_2_weight'])
        arg_params['fc_new_23_bias'] = mx.nd.zeros(shape=self.arg_shape_dict['fc_new_2_bias'])
        arg_params['cls_score3_weight'] = mx.random.normal(0, 0.01, shape=self.arg_shape_dict['cls_score_weight'])
        arg_params['cls_score3_bias'] = mx.nd.zeros(shape=self.arg_shape_dict['cls_score_bias'])
        arg_params['bbox_pred3_weight'] = mx.random.normal(0, 0.01, shape=self.arg_shape_dict['bbox_pred_weight'])
        arg_params['bbox_pred3_bias'] = mx.nd.zeros(shape=self.arg_shape_dict['bbox_pred_bias'])
