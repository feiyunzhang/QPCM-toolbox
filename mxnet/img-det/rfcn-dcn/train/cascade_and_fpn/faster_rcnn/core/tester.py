...
def im_detect(predictor, data_batch, data_names, scales, cfg):
    ...
    for output, data_dict, scale in zip(output_all, data_dict_all, scales):
        if cfg.TEST.HAS_RPN:
            rois = output['rois3_proposals'].asnumpy()[:, 1:]
        else:
            rois = data_dict['rois3'].asnumpy().reshape((-1, 5))[:, 1:]
        im_shape = data_dict['data'].shape

        # save output
        scores = output['cls_prob_reshape3_output'].asnumpy()[0]
        bbox_deltas = output['bbox_pred_reshape3_output'].asnumpy()[0]
