...
void VideoDataLayer<Dtype>:: DataLayerSetUp(const vector<Blob<Dtype>*>& bottom, const vector<Blob<Dtype>*>& top){
    ...
    //modified by zxt at time 20180321
    const bool multi_scale  = this->layer_param_.video_data_param().multi_scale();
    const int smin = this->layer_param_.video_data_param().smin();
    const int smax = this->layer_param_.video_data_param().smax();
    const bool contrast_shift = this->layer_param_.video_data_param().contrast_shift();
    const float cp1 = this->layer_param_.video_data_param().contrast_p1();
    const float cp2 = this->layer_param_.video_data_param().contrast_p2();
    const bool brightness_shift = this->layer_param_.video_data_param().brightness_shift();
    const float bp1 = this->layer_param_.video_data_param().brightness_p1();
    const float bp2 = this->layer_param_.video_data_param().brightness_p2();
    const bool color_shift = this->layer_param_.video_data_param().color_shift();
    const float color_std = this->layer_param_.video_data_param().color_std();
    //modified by txz at time 20180321
	...
	if (this->layer_param_.video_data_param().modality() == VideoDataParameter_Modality_FLOW)
		...
		CHECK(ReadSegmentRGBToDatum(lines_[lines_id_].first, lines_[lines_id_].second,
									offsets, new_height, new_width, new_length, &datum, is_color, name_pattern_.c_str(),
									multi_scale, new_scale, contrast_shift, cp1, cp2, brightness_shift, 
									bp1, bp2, color_shift, color_std)); 
 		//modified by txz at time 20180321
	...
	for (int item_id = 0; item_id < batch_size; ++item_id){
		...
		if (this->layer_param_.video_data_param().modality() == VideoDataParameter_Modality_FLOW){
			...
			if(!ReadSegmentRGBToDatum(lines_[lines_id_].first, lines_[lines_id_].second,
									  offsets, new_height, new_width, new_length, &datum, is_color, name_pattern_.c_str(), multi_scale, new_scale,
