...
import json
import urllib
from  inference_config_merge import  config
...
def postProcess(output=None, imagePath=None):
    ...
    result_dict['Top-1 Class'] = str(cls_label_dict[int(index_list[-1])].split(' ')[0])
...
def infereneAllImage(net_cls=None, imageList=None, urlFlag=None):
    ...
    print("one image : only inference time is : %f" % (only_inference_time/len(imageList)))
