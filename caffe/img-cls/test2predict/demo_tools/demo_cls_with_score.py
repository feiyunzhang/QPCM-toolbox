...
def classify_flow(url,label_cls, score_cls):
    flow = {
        "url": url,
        "type": "image",
        "label": {"class": {"terror":label_cls, "score": score_cls}}
    }
    return flow
    ...
    with open(urllist_filePath,'r') as read_file,open(saveJsonFile,'w') as write_file:
        for url_line in read_file.readlines():
            ...
            if np.shape(origimg) != ():
                ...
                cla_index = str(label_cls[0])
                json_dict = classify_flow(url_line.strip(),cls_dict.get(cla_index) if float(score_cls[0]) > Config.CLS_CONFIDENCE_THRESH else "others", float(score_cls[0]))
