...
def single_img_process(net_cls, im_path, ori_img, label_list):
    ...
    if np.shape(img) != ():
        ...
        #print output_prob
        result_dict['Top-1 Index'] = int(label_list[int(index_list[-1])].split('_')[0])

        result_dict['Top-1 Class'] = label_list[int(index_list[-1])].split(' ')[0]
        ...
        print corres_prob
        result_dict['Confidence'] = [str(i) for i in list(corres_prob)]
...
if __name__ == '__main__':
    ...
    cls_list = np.loadtxt('lib/correspondence.lst', str, delimiter='\n')
    ...
    log_result = open('correspond_log.json', 'w')
