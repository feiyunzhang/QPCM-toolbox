...
def parse_kinetics_splits():
    class_ind = [x.strip().split() for x in open('data/kinetics400_splits/classInd.txt')]
    class_mapping = {x[1]:int(x[0])-1 for x in class_ind}
    ...
    splits = []
    for i in xrange(1, 2):
        train_list = [line2rec(x) for x in open('data/kinetics400_splits/mini_testlist{:02d}.txt'.format(i))]
        test_list = [line2rec(x) for x in open('data/kinetics400_splits/mini_testlist{:02d}.txt'.format(i))]
        splits.append((train_list, test_list))
    return splits
