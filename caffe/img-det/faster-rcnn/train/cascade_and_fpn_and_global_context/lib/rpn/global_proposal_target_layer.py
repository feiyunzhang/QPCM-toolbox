...
class GlobalProposalTargetLayer(caffe.Layer):
    """
    ...
    def forward(self, bottom, top):
        ...
        ##zxt##
        global_all_rois = np.vstack(
            (global_all_rois, np.hstack((zeros, gt_boxes[:, :-1])))
        )
        ...
        ##zxt##
        labels, rois, global_rois, bbox_targets, bbox_inside_weights = _sample_rois(
            all_rois, global_all_rois, gt_boxes, fg_rois_per_image,
        ...
        # sampled global_rois
        # modified by zxt
        global_rois = global_rois.reshape((global_rois.shape[0], global_rois.shape[1], 1, 1))
        top[1].reshape(*global_rois.shape)
        top[1].data[...] = global_rois
...
def _sample_rois(all_rois, global_all_rois, gt_boxes, fg_rois_per_image, rois_per_image, num_classes):
    ...
    ##zxt##
    global_rois = global_all_rois[keep_inds]
    ...
    return labels, rois, global_rois, bbox_targets, bbox_inside_weights
