...
class ProposalTargetLayer(caffe.Layer):
    ...
    def setup(self, bottom, top):
        ...
        self._stage_num = layer_params['stage_num']
    ...
    def forward(self, bottom, top):
        ...
        if self._stage_num == 'stage_1st':
            labels, rois, bbox_targets, bbox_inside_weights = _sample_rois(
                all_rois, gt_boxes, fg_rois_per_image,
                rois_per_image, self._num_classes)
        elif self._stage_num == 'stage_2nd':
            labels, rois, bbox_targets, bbox_inside_weights = _sample_rois_2nd(
                all_rois, gt_boxes, fg_rois_per_image,
                rois_per_image, self._num_classes)
        elif self._stage_num == 'stage_3rd':
            labels, rois, bbox_targets, bbox_inside_weights = _sample_rois_3rd(
                all_rois, gt_boxes, fg_rois_per_image,
                rois_per_image, self._num_classes)
...
def _sample_rois_2nd(all_rois, gt_boxes, fg_rois_per_image, rois_per_image, num_classes):
    """Generate a random sample of RoIs comprising foreground and background
    examples.
    """
    # overlaps: (rois x gt_boxes)
    ...
    # Select foreground RoIs as those with >= FG_THRESH overlap
    fg_inds = np.where(max_overlaps >= cfg.TRAIN.FG_THRESH_Stage_2nd)[0]
    # Guard against the case when an image has fewer than fg_rois_per_image
    # foreground RoIs
    fg_rois_per_this_image = int(min(fg_rois_per_image, fg_inds.size))
    # Sample foreground regions without replacement
    ...
    # Select background RoIs as those within [BG_THRESH_LO, BG_THRESH_HI)
    bg_inds = np.where((max_overlaps < cfg.TRAIN.BG_THRESH_HI_Stage_2nd) &
                       (max_overlaps >= cfg.TRAIN.BG_THRESH_LO_Stage_2nd))[0]
    # Compute number of background RoIs to take from this image (guarding
    # against there being fewer than desired)
    bg_rois_per_this_image = rois_per_image - fg_rois_per_this_image
    bg_rois_per_this_image = int(min(bg_rois_per_this_image, bg_inds.size))
...
def _sample_rois_3rd(all_rois, gt_boxes, fg_rois_per_image, rois_per_image, num_classes):
    """Generate a random sample of RoIs comprising foreground and background
    examples.
    """
    # overlaps: (rois x gt_boxes)
    ...
    # Select foreground RoIs as those with >= FG_THRESH overlap
    fg_inds = np.where(max_overlaps >= cfg.TRAIN.FG_THRESH_Stage_3rd)[0]
    # Guard against the case when an image has fewer than fg_rois_per_image
    # foreground RoIs
    fg_rois_per_this_image = int(min(fg_rois_per_image, fg_inds.size))
    # Sample foreground regions without replacement
    ...
    # Select background RoIs as those within [BG_THRESH_LO, BG_THRESH_HI)
    bg_inds = np.where((max_overlaps < cfg.TRAIN.BG_THRESH_HI_Stage_3rd) &
                       (max_overlaps >= cfg.TRAIN.BG_THRESH_LO_Stage_3rd))[0]
    # Compute number of background RoIs to take from this image (guarding
    # against there being fewer than desired)
    bg_rois_per_this_image = rois_per_image - fg_rois_per_this_image
    bg_rois_per_this_image = int(min(bg_rois_per_this_image, bg_inds.size))
