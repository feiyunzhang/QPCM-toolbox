...
def normalize(im, mean=(0.0, 0.0, 0.0), std=(1.0, 1.0, 1.0), rgb=True):
    if rgb:
        r, g, b = cv2.split(im)
    else:
        b, g, r = cv2.split(im)
    norm_im = cv2.merge([(r - 255*mean[0]) / 255*std[0], (g - 255*mean[1]) / 255*std[1], (b - 255*mean[2]) / 255*std[2]])
    return norm_im
...
class preproc(object):
    def __init__(self, resize_wh, means, std, p):
        self.means = means
        self.std = std
